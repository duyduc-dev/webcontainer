...
function findDescriptor(obj: any, prop: string): PropertyDescriptor | undefined {
  let cur = obj;
  while (cur) {
...
function resolvePrototypeMember(proto: any, methodPart: string): unknown {
  // Well-known symbol member, e.g. PrototypeSymbolReplace -> proto[Symbol.replace].
  if (methodPart.startsWith("Symbol")) {
    const sym = (Symbol as any)[lowerFirst(methodPart.slice(6))];
  ...
  if (methodPart.startsWith("Get")) {
    ...
    if (d?.get) return uncurryThis(d.get);
  }
  if (methodPart.startsWith("Set")) {
    const d = findDescriptor(proto, lowerFirst(methodPart.slice(3)));
    if (d?.set) return uncurryThis(d.set);
  }
  return undefined;
}
...
const SafeStringPrototypeSearch = (str: string, regexp: RegExp): number => {
  regexp.lastIndex = 0;
...
const SafePromiseAll = (promises: unknown[], mapFn?: AnyFn) =>
...
const SPECIALS: Record<string, unknown> = {
  ...
  SymbolDispose: (Symbol as any).dispose ?? Symbol("nodejs.dispose"),
  SymbolAsyncDispose: (Symbol as any).asyncDispose ?? Symbol("nodejs.asyncDispose"),
};
...
const cache: Record<string, unknown> = { __proto__: null } as Record<string, unknown>;

const primordials: Record<string, any> = new Proxy(
  { __proto__: null } as Record<string, unknown>,
  {
    get(_target, prop) {
      ...
      if (value === undefined && !["undefined", "NaN"].includes(prop)) {
        throw new Error(`primordials.${prop} is not resolvable — add it to runtime/node/primordials.ts`);
      }
      cache[prop] = value;
      return value;
    },
