...
//
// This is where vendoring real Node source pays for itself: Node's JS Buffer
// (vendored verbatim, later phase) calls down here for the things V8/C++
// normally provide — utf8/base64/hex/ucs2 codecs, byte search, compare/copy/
...
// is why this only bites in-browser. Copy the range into a fresh, non-shared
// buffer before decoding in that case; the manual latin1/ascii/hex/ucs2 slices
// index the bytes directly and are unaffected.
const hasSAB = typeof SharedArrayBuffer !== "undefined";
const unshare = (view: Uint8Array): Uint8Array =>
...
function asBytes(input: ByteLike): Uint8Array {
...
function encodeBase64(buf: Uint8Array, start: number, end: number, url: boolean): string {
...
const utf8Slice = (buf: Uint8Array, start: number, end: number): string =>
  utf8Decoder.decode(unshare(buf.subarray(start, end)));
...
function hexSlice(buf: Uint8Array, start: number, end: number): string {
  let s = "";
  for (let i = start; i < end; i++) s += buf[i]!.toString(16).padStart(2, "0");
  return s;
}

function ucs2Slice(buf: Uint8Array, start: number, end: number): string {
  let s = "";
...
const base64Slice = (buf: Uint8Array, start: number, end: number): string => encodeBase64(buf, start, end, false);
const base64urlSlice = (buf: Uint8Array, start: number, end: number): string => encodeBase64(buf, start, end, true);

// ------------------------------------------------------------------ writes ---
// (buf, string, offset, length) -> bytesWritten

function utf8WriteStatic(buf: Uint8Array, string: string, offset: number, length: number): number {
  const { written } = encoder.encodeInto(string, buf.subarray(offset, offset + length));
  return written ?? 0;
}

function asciiWriteStatic(buf: Uint8Array, string: string, offset: number, length: number): number {
  const n = Math.min(string.length, length);
  for (let i = 0; i < n; i++) buf[offset + i] = string.charCodeAt(i) & 0x7f;
  return n;
}

function latin1WriteStatic(buf: Uint8Array, string: string, offset: number, length: number): number {
  const n = Math.min(string.length, length);
  for (let i = 0; i < n; i++) buf[offset + i] = string.charCodeAt(i) & 0xff;
  return n;
}

function ucs2Write(buf: Uint8Array, string: string, offset: number, length: number): number {
...
function hexWrite(buf: Uint8Array, string: string, offset: number, length: number): number {
...
function base64Write(buf: Uint8Array, string: string, offset: number, length: number): number {
...
const compare = (a: Uint8Array, b: Uint8Array): number => compareRange(a, 0, a.length, b, 0, b.length);
const compareOffset = (
  source: Uint8Array,
  target: Uint8Array,
  targetStart: number,
  ...
  targetEnd: number,
  sourceEnd: number,
): number => compareRange(source, sourceStart, sourceEnd, target, targetStart, targetEnd);

// -------------------------------------------------------------------- misc ---

function copy(source: Uint8Array, target: Uint8Array, targetStart: number, sourceStart: number, nb: number): number {
  target.set(source.subarray(sourceStart, sourceStart + nb), targetStart);
  return nb;
}

function bindingFill(buf: Uint8Array, value: string | ByteLike, offset: number, end: number, encoding?: string): number {
  const bytes = typeof value === "string" ? encodeString(value, normalizeForFill(encoding)) : asBytes(value);
  if (!bytes || bytes.length === 0) return -1;
  for (let i = offset, j = 0; i < end; i++, j++) buf[i] = bytes[j % bytes.length]!;
  return end - offset;
}

function normalizeForFill(encoding?: string): string {
  if (!encoding) return "utf8";
  const e = String(encoding).toLowerCase();
  return e === "utf-8" ? "utf8" : e;
}

function encodeString(str: string, enc: string): Uint8Array {
  switch (enc) {
    case "utf8":
    case "utf-8":
...
function byteSearch(haystack: Uint8Array, needle: Uint8Array, byteOffset: number, dir: boolean): number {
  const len = haystack.length;
  const nlen = needle.length;
  if (nlen === 0) return dir ? Math.max(0, Math.min(byteOffset, len)) : len;
  if (nlen > len) return -1;
  if (dir) {
    const start = byteOffset < 0 ? Math.max(len + byteOffset, 0) : byteOffset;
...
const indexOfBuffer = (buf: Uint8Array, val: ByteLike, byteOffset: number, _encoding: string, dir: boolean): number =>
  byteSearch(buf, asBytes(val), byteOffset, dir);

function indexOfNumber(buf: Uint8Array, val: number, byteOffset: number, dir: boolean): number {
...
const indexOfString = (buf: Uint8Array, val: string, byteOffset: number, encoding: string, dir: boolean): number =>
  byteSearch(buf, encodeString(val, normalizeForFill(encoding)), byteOffset, dir);

// ---------------------------------------------------------------- byteswap ---

function swap16(buf: Uint8Array): Uint8Array {
...
function swap32(buf: Uint8Array): Uint8Array {
...
function swap64(buf: Uint8Array): Uint8Array {
...
function btoa(str: string): string | -1 {
  const b = new Uint8Array(str.length);
  for (let i = 0; i < str.length; i++) {
    const c = str.charCodeAt(i);
    if (c > 0xff) return -1;
...
function atob(str: string): string | -2 {
  let clean = "";
  for (let i = 0; i < str.length; i++) {
    const c = str[i]!;
    if (c === " " || c === "\t" || c === "\n" || c === "\r" || c === "\f") continue;
...
const createBufferBinding = () => ({
  ...
  byteLengthUtf8: (str: string) => encoder.encode(str).length,
  compare,
  compareOffset,
  copy,
  fill: bindingFill,
  isAscii: (input: ByteLike) => {
    const b = asBytes(input);
    for (let i = 0; i < b.length; i++) if (b[i]! > 127) return false;
    return true;
  },
  isUtf8: (input: ByteLike) => {
    try {
