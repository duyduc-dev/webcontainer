...
const isIPv6 = (s: unknown): s is string => {
...
/** Parse a dotted-quad into its 4 bytes. Throws for anything else. */
const parseIPv4 = (addr: string): Uint8Array => {
...
const parseIPv6 = (addr: string): Uint8Array => {
...
/** Format 16 bytes the way RFC 5952 requires (lower-case hex, no leading
 * zeros, longest run of zero hextets collapsed to `::`). */
const formatIPv6 = (bytes: Uint8Array): string => {
  // RFC 5952 §5: an IPv4-mapped address keeps its dotted-quad tail.
...
export { formatIPv4, formatIPv6, isIPv4, isIPv6, parseIPv4, parseIPv6 };
