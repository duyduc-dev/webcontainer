// tls — partial shim.
//
// There is no TLS backend, so an actual handshake / secure connection can't
// work. But real code (and its deep deps) routinely *construct* a
// `tls.TLSSocket` or call `tls.createSecureContext()` at module-load time for
// feature detection or prototype access — a real TLSSocket extends net.Socket
// and does NO I/O until a handshake starts. So construction must be benign;
...
export default function (exports, require, module) {
  ...
  const notImpl = (what) => {
    const err = new Error("TLS is not implemented yet" + (what ? " (" + what + ")" : ""));
  ...
  // JSStreamSocket class. Some transitive deps read exactly that at
  // module-load time. Give the benign socket a synthetic handle so the lookup
  // yields a harmless class instead of throwing on null.
