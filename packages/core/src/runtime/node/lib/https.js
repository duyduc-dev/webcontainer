// https — a fetch-backed client.
//
// Real TLS/sockets don't exist in a Worker, so instead of a socket-level https
// we implement https.request/get on top of the Fetcher Worker bridge
// (globalThis.__dwcFetchAsync). This is enough for the real npm
...
//
// The transport itself lives in internal/fetch-transport.js: `http` needs the
// same fetch-backed client (there is no in-VM loopback net yet either), and one
// implementation shared by both protocols beats two copies that drift. This
// module is the https-shaped shell around it — the scheme/port defaults, the
// no-op Agent, and the absent https *server*.
//
...
export default function (exports, require, module, process, internalBinding, primordials) {
  ...
  const notImpl = () => {
    const err = new Error("in-VM https servers are not supported");
