...
//
// sha512 specifically is NOT optional the way the rest of this list is: real
// npm's own `ssri` (subresource-integrity) dependency defaults to sha512 for
// every package's content-addressable cache key AND for verifying a
// tarball's registry-provided `dist.integrity` field - every single package
// install computes one, not a narrow one-off caller.
//
// Deliberately NOT implemented (throws loudly rather than silently degrading):
// sha224/sha384 and any other digest, ciphers (createCipheriv/
...
export default function (exports, require, module, process, internalBinding, primordials) {
  ...
  // SHA-512's 64-bit word arithmetic doesn't fit in JS's 32-bit bitwise ops
  // (unlike md5/sha1/sha256 above) - BigInt gives exact 64-bit values
  // directly, at the cost of needing an explicit `& MASK64` after every
  // addition/rotation (BigInt arithmetic is arbitrary-precision, so it
  // doesn't wrap on its own the way `| 0` does for the 32-bit cores).
  const MASK64 = (1n << 64n) - 1n;
  const rotr64 = (x, n) => ((x >> BigInt(n)) | (x << (64n - BigInt(n)))) & MASK64;
  const H512 = [
  ...
  function sha512(bytes) {
    ...
    msg[len] = 0x80;
    const dv = new DataView(msg.buffer);
    dv.setBigUint64(total - 8, BigInt(len) * 8n, false);
    // the high 64 bits of the 128-bit length field stay zero: no message
    // here ever approaches 2^64 bits.

    let [h0, h1, h2, h3, h4, h5, h6, h7] = H512;
    ...
    const odv = new DataView(out.buffer);
    [h0, h1, h2, h3, h4, h5, h6, h7].forEach((v, i) => odv.setBigUint64(i * 8, v, false));
    return out;
  }

  // sha512's HMAC block size is 128 bytes, not the 64 the other three share.
  const CORES = {
    md5: { fn: md5, blockSize: 64 },
    sha1: { fn: sha1, blockSize: 64 },
    sha256: { fn: sha256, blockSize: 64 },
    sha512: { fn: sha512, blockSize: 128 },
  };
